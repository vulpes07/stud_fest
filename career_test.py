...
from stud_reg import is_admin, is_registered
...
@router_test.callback_query(lambda c: c.data.startswith(("yes_", "no_")))
async def handle_answer(callback: types.CallbackQuery, state: FSMContext):
    ...
    if index < len(question_fields):
        ...
        question_text = question_fields[index][0]
        await callback.message.edit_text(question_text, reply_markup=yes_no_kb(index))
    else:
        await state.clear()
        ...
        prompt = (
    "Ты — опытный профориентолог. Пользователь ответил на тест. Вот направления, которые его интересуют:\n\n"
    f"{', '.join(interests)}\n\n"
    "На основе только его интересов и ответов подбери для него подходящие факультеты и специальности с этих факультетов, которые больше всего его интересуют "
    "из следующего списка (не выходи за его рамки):\n\n" +
    "\n".join([f"{k}: {', '.join(v)}" for k, v in specialties.items()]) +
    "\n\n"
    "Дай краткий, понятный ответ на русском языке. Ничего не придумывай вне списка."
)
